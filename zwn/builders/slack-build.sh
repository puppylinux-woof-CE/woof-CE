...
VERSION=${VERSION:-slackware-14.2}
...
add_repo() {
	...
	#--
	zversion=$2
	[ -z "$zversion" ] && zversion="$VERSION"
	#--
	zrepos=$3
	[ -z "$zrepos" ] && zrepos="[main]"
	#--
	for p in $(echo $zrepos | tr ':' ' '); do
		version_equals_repo=
		if [ "$p" = "[main]" ] ; then
			#bug: version = repos-to-use
			version_equals_repo=1
			p=sl-${1##*/}
			MARKER="### $zversion-$p-$1 ###" 
			localdb=$zversion-$p-$4
			pkgdb_url="$(echo $REPO_PKGDB_URL | sed "s|%repo_url%|$1|; s|%version%|$2|; s|%repo%||; s|%repo_pkgdb%|$4|;")"
		else
			MARKER="### $zversion-$p-$1 ###" 
			localdb=$zversion-$p-$4
			pkgdb_url="$(echo $REPO_PKGDB_URL | sed "s|%repo_url%|$1|; s|%version%|$zversion|; s|%repo%|$p|; s|%repo_pkgdb%|$4|;")"
		fi
		pkgdb_url=${pkgdb_url/\/\/\//\/}
		# download
		...
		if [ -z "$DRY_RUN" ] ; then
			if [ "$version_equals_repo" ] ; then
				echo "SOURCE=${pkgdb_url%/*}:OFFICIAL" >> $CHROOT_DIR/$SLAPTGET_PKGDB
			else
				echo "SOURCE=${pkgdb_url%/*/*}:OFFICIAL" >> $CHROOT_DIR/$SLAPTGET_PKGDB
			fi
		fi
		...
		if ! grep -F -m1 -q "$MARKER" $REPO_DIR/$LOCAL_PKGDB 2>/dev/null; then	
			echo Processing database for "$zversion $p" ...
			if [ "$version_equals_repo" ] ; then
				repo_url="${1}/$2"
				p=
			else
			...
			< "$REPO_DIR/$localdb" \
			awk -v repo_url=$repo_url -v section=$p '
/\.t.z$/ {
	PKGMD5=$1; 
	sub(/\.\//,"",$2); PKGPATH=$2; 
	sub(/.*\//,"",$2); PKGFILE=$2; 
...
add_multiple_repos() {
	while [ "$1" ]; do
		IFS="|" read zurl zversion zsections zpkgdb <<< "$1" # <<< = bashism
		add_repo "$zurl" "$zversion" "$zsections" "$zpkgdb"
		shift
	done
}
...
get_pkg_info() {
	...
	if [ -z "$1" ] ; then
		set --  $(grep -m1 "|${pkg}.t.z|" $REPO_DIR/$LOCAL_PKGDB)
	fi
