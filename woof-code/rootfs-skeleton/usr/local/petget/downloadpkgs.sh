...
if [ -s /tmp/petget_proc/petget-installed-pkgs-log ];then
 [ -f /tmp/petget_proc/install_quietly ] && FLAGPET='yes'
 ...
 INSTALLEDMSG="`cat /tmp/petget_proc/petget-installed-pkgs-log`" #ex line: "PACKAGE: langpack_ru-20120720 CATEGORY: Setup"
...
" 
 ...
 if [ "$EXIT" = "BUTTON_TRIM_FAT" ];then
   ...
   do
    ...
    if [ "$ENTRY_LOCALE" != "" ];then
     if [ "`echo -n "$ONEFILE" | grep --extended-regexp '/locale/|/nls/|/i18n/' | grep -v -E "$elPATTERN"`" != "" ];then
      rm -f "$ONEFILE"
      grep -v "$ONEFILE" /root/.packages/${PKGNAME}.files > /tmp/petget_proc/petget_pkgfiles_temp
  ...
  [ ! -f /tmp/petget_proc/install_quietly ] && kill $X4PID || echo
 fi
