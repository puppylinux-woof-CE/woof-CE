...
#/tmp/petget_proc/petget_installpreview_pkgname (writen in installpreview.sh) has name of package being
...
#/tmp/petget_proc/petget_installed_patterns_all (writen in findmissingpkgs.sh) has a list of already installed
...
#120831 npierce, jemimah: findmissingpkgs.sh creates /tmp/petget_proc/petget_missingpkgs_patterns and /tmp/petget_proc/petget_missingpkgs_patterns_with_versioning, need to use latter here.
...
ALLINSTALLEDPKGS="`cat /tmp/petget_proc/petget_installed_patterns_all`"
TREE1="`cat /tmp/petget_proc/petget_installpreview_pkgname`"
...
SIZE2=0 #120907
echo -n "" > /tmp/petget_proc/petget_missingpkgs_patterns_acc #120903
echo -n "" > /tmp/petget_proc/petget_missingpkgs_patterns_acc0 #120903
...
do
 ...
 echo "" >> /tmp/petget_proc/petget_deps_visualtreelog #w017
 ...
 do
  ...
  do
   ...
   if [ "$DB_dependencies" != "" ];then
    ...
    echo "$ALLDEPS_PATTERNS" > /tmp/petget_proc/petget_subpkg_deps_patterns
    cp -f /tmp/petget_proc/petget_installed_patterns_all /tmp/petget_proc/petget_installed_patterns_allxx #120907
    cat /tmp/petget_proc/petget_missingpkgs_patterns_acc >> /tmp/petget_proc/petget_installed_patterns_allxx #120907 greatly speeds search, improves hierarchy view.
    ...
    case $ONELEVEL in
     ...
     3)  echo "        $ONEDEP: $MISSDEPSLIST" >> /tmp/petget_proc/petget_deps_visualtreelog ;;
     4)  echo "            $ONEDEP: $MISSDEPSLIST" >> /tmp/petget_proc/petget_deps_visualtreelog ;;
     5)  echo "                $ONEDEP: $MISSDEPSLIST" >> /tmp/petget_proc/petget_deps_visualtreelog ;;
     6)  echo "                    $ONEDEP: $MISSDEPSLIST" >> /tmp/petget_proc/petget_deps_visualtreelog ;;
     7)  echo "                        $ONEDEP: $MISSDEPSLIST" >> /tmp/petget_proc/petget_deps_visualtreelog ;;
     8)  echo "                            $ONEDEP: $MISSDEPSLIST" >> /tmp/petget_proc/petget_deps_visualtreelog ;;
     9)  echo "                            $ONEDEP: $MISSDEPSLIST" >> /tmp/petget_proc/petget_deps_visualtreelog ;;
     10) echo "                                $ONEDEP: $MISSDEPSLIST" >> /tmp/petget_proc/petget_deps_visualtreelog ;;
     11) echo "                                    $ONEDEP: $MISSDEPSLIST" >> /tmp/petget_proc/petget_deps_visualtreelog ;;
     12) echo "                                        $ONEDEP: $MISSDEPSLIST" >> /tmp/petget_proc/petget_deps_visualtreelog ;;
     13) echo "                                            $ONEDEP: $MISSDEPSLIST" >> /tmp/petget_proc/petget_deps_visualtreelog ;;
    esac
    break
   fi
  done
 ...
 cat /tmp/petget_proc/petget_missingpkgs_patternsx >> /tmp/petget_proc/petget_missingpkgs_patterns_acc0 #accumulate them.
 sort -u /tmp/petget_proc/petget_missingpkgs_patterns_acc0 > /tmp/petget_proc/petget_missingpkgs_patterns_acc
 #120907 get out of loop when no more missing deps found...
 SIZE1=$(cat /tmp/petget_proc/petget_missingpkgs_patterns_acc-prev | wc -l)
...
grep -v --file=/tmp/petget_proc/petget_missingpkgs_patterns /tmp/petget_proc/petget_missingpkgs_patterns_acc > /tmp/petget_proc/petget_missingpkgs_patterns_accx #make sure that 2nd file does not have any level1 deps.
mv -f /tmp/petget_proc/petget_missingpkgs_patterns_accx /tmp/petget_proc/petget_missingpkgs_patterns_acc
sed -e 's%|%\&%g' -e 's%^\&%|%' -e 's%\&$%|%' /tmp/petget_proc/petget_missingpkgs_patterns_with_versioning >> /tmp/petget_proc/petget_missingpkgs_patterns_acc
sort -u /tmp/petget_proc/petget_missingpkgs_patterns_acc > /tmp/petget_proc/petget_missingpkgs_patternsx
mv -f /tmp/petget_proc/petget_missingpkgs_patternsx /tmp/petget_proc/petget_missingpkgs_patterns_and_versioning_level1
...
do
 ...
 if [ -f /tmp/petget_proc/petget_missing_dbentries-${DBFILE}-2 ];then
  sort -u /tmp/petget_proc/petget_missing_dbentries-${DBFILE}-2 > /tmp/petget_proc/petget_missing_dbentries-${DBFILE}
  rm -f /tmp/petget_proc/petget_missing_dbentries-${DBFILE}-2
 fi
done
...
[ ! -f /tmp/petget_proc/install_quietly ] && kill $X1PID || exit 0
