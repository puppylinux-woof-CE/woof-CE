...
# add nano test
chroot bdrv apt-get update
chroot bdrv apt -y install nano
chroot bdrv apt-get install -y ca-certificates
chroot bdrv wget --no-check-certificate https://dl.jami.net/public-key.gpg -O /usr/share/keyrings/jami-archive-keyring.gpg
...
chroot bdrv cp /usr/lib/x86_64-linux-gnu/libjsoncpp.so.24 /lib/
...
chroot bdrv cp /etc/ccs/dring.yml /root/.config/jami/
